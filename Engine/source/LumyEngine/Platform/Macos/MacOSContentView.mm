...
static KeyCode TranslateKey(UInt16 key);

@implementation MacOSContentView
...
{
    Inputs::ProcessButton(MouseButton::Left, true);
}
...
{
    Inputs::ProcessButton(MouseButton::Left, false);
}

- (void)mouseMoved:(NSEvent*)event
{
    NSPoint position = [event locationInWindow];
    Inputs::ProcessMouseMove(position.x, position.y);
}

- (void)rightMouseDown:(NSEvent*)event
{
    Inputs::ProcessButton(MouseButton::Right, true);
}
...
{
    Inputs::ProcessButton(MouseButton::Right, false);
}
...
- (void)updateTrackingAreas
{
    ...
    //     NSTrackingEnabledDuringMouseDrag | NSTrackingCursorUpdate | NSTrackingInVisibleRect |
...
{
    const KeyCode key = TranslateKey([event keyCode]);
    Inputs::ProcessKey(key, false);
}

- (void)scrollWheel:(NSEvent*)event
{
    Inputs::ProcessMouseScroll([event scrollingDeltaX], [event scrollingDeltaY]);
}
...
{
    const NSRect frame = [[window contentView] frame];
